...
def run_thread_loop(agent, map_name, visualize, step):
  count=0
  scores=[]
  with sc2_env.SC2Env(
    map_name=map_name,
    ...
    for recorder, is_done in run_loop([agent], env, MAX_AGENT_STEPS):
      if FLAGS.training:
        ...
        if(count>=5):
          f=open('log_loop', 'a')          
          f.write(str(step)+ ' ' +str(np.average(np.array(scores)))+' '+str(np.std(np.array(scores))) + '\n')
          f.close()
          return
    if FLAGS.save_replay:
      env.save_replay(agent.name)
...
def _main(unused_argv):
  ...
  if not FLAGS.training or FLAGS.continuation:
    ...
    max_step=47001     
    step=min_step
    ckpt = tf.train.get_checkpoint_state(SNAPSHOT)
    path_1=ckpt.model_checkpoint_path.split('-')[0] + '-'
    while(step<=max_step):
      path_full=path_1 + str(int(step))
      agent.load_model_loop(path_full)
      run_thread_loop(agents[-1], FLAGS.map, FLAGS.render, step)
      step+=1000
